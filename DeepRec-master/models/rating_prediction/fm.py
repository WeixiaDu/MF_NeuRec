...
class FM():
    ...
    def build_network(self, feature_M, num_factor = 64):
        ...
        self.summed_features_embedding = tf.reduce_sum(nonzero_embeddings, 1)#按行求和
        ...
        bilinear = tf.reduce_sum(self.FM, 1, keep_dims=True)#按行求和
        self.f_b = tf.reduce_sum(tf.nn.embedding_lookup(self.feature_bias, self.train_features), 1)
        b = self.bias * tf.ones_like(self.y)
        self.pred_rating = tf.add_n([bilinear, self.f_b, b])#实现一个列表的元素的相加。就是输入的对象是一个列表，
		#列表里的元素可以是向量，矩阵，等
